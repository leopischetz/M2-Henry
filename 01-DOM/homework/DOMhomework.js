...
name.innerHTML = name.innerHTML + yo;  
...
function buildToDos(toDos) {
  // Tu código acá:
  var array = toDos.map(function(toDos,index){return buildToDo(toDos, index)});
  return array;
}
...
function displayToDos() {
  ...
  var nArray = buildToDos(toDoItems);
  for(var i=0; i<nArray.lenght; i++){
    toDoContainer.appendChild(nArray[i]);
  };
}
...
document.getElementById('addButton').addEventListener('mouseenter',addToDo);
